...
const backendOutput = {
    ...
    MONGO_URI: 'mongodb://mongo:jkrwhMsJFLNcUrljsbMIZVuSJYtzAEiH@autorack.proxy.rlwy.net:55715',
    MONGO_DATABASE: 'RTC',
    MAILER_ENABLED: false,
};

let frontendOutput = {
    VITE_SITE_TITLE: 'Blink',
    VITE_BACKEND_URL: 'http://localhost',
    VITE_DEMO: false,
    VITE_SITE_BRAND: 'MontaCoder',
...
(async () => {
    ...
    console.log("");
    console.log("MontaCoder".yellow);
    console.log(`Blink v${pkg.version} Installer`.yellow);
    console.log("");
    ...
    if (arg === 'setup') {
        ...
        console.log('Blink needs to know the public ip address of your machine.'.cyan);
        ...
        console.log('');
        console.log('Blink requires a domain, such as elderberry.example.com.'.cyan);
    ...
    if (['setup', 'rebuild'].includes(arg)) {
        ...
        if (!os.includes('Ubuntu')) {
            ...
            console.log('If you are running this on Ubuntu, please contact MontaCoder Support.');
        ...
        if (!version.includes('22.04') && !version.includes('20.04') && !version.includes('18.04')) {
            ...
            console.log('If you are running this on a correct version, please contact MontaCoder Support.');
            return process.exit(0);
        }
        ...
        console.log(`${arg === 'setup' ? 'installing' : 'rebuilding'} Blink backend...`.yellow);
        ...
        try {
            exec('pm2 delete --silent Blink', {stdio : 'pipe'});
        } catch (e) {}
        exec('cd ../backend && pm2 start index.js --name Blink');
        exec('pm2 save');
        exec('pm2 startup');
        console.log('Blink backend started'.green);

        console.log('');

        console.log(`${arg === 'setup' ? 'installing' : 'rebuilding'} Blink frontend...`.yellow);
        ...
        console.log('Blink frontend ok'.green);
    }
    ...
    if (arg === 'restart') {
        try {
            exec('pm2 restart Blink', {stdio : 'pipe'});
        } catch (e) {}
    }

    if (arg === 'stop') {
        try {
            exec('pm2 stop Blink', {stdio : 'pipe'});
        } catch (e) {}
    }
    ...
    if (arg === 'stop') {
        console.log("");
        console.log(`Blink has been stopped.`.green);
        console.log("");
    } else {
        console.log("");
        console.log(`Blink v${pkg.version} ${arg === 'setup' ? 'setup' : 'restart'} complete!`.green);
        if (arg === 'setup') {
            console.log(`You should now be able to access Blink at ${domain}`);
        }
        console.log("");
    }
})();
